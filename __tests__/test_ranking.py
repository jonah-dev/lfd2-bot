...
from plugins.SuperCashBrosLeftForDead.plugin import leaderboard
...
class TestRanking(AsyncTestCase):
    async def test_send_leaderboard(self):
        topic = f"@SuperCashBrosLeftForDead(history: '{SHEETS}')"
        lobby = Lobby(bot(), ctx := channel(topic=topic))
        await leaderboard(lobby, ctx)
        assert ctx.send.await_count == 1
        embed: Embed = ctx.send.await_args.kwargs["embed"]
        assert len(embed.fields) == 31

    async def test_print_ranks(self):
